...
clone_repo(){
    echo "进行初始化。。。"
    rm -rf IBMYes
    git clone https://github.com/cyberxsboy/IBMYes
    cd IBMYes
    ...
    # Script from https://github.com/cyberxsboy/fhs-install-v2ray/blob/master/install-release.sh
    # Get V2Ray release version number
    TMP_FILE="$(mktemp)"
    if ! curl -s -o "$TMP_FILE" 'https://github.com/cyberxsboy/v2fly_v2ray-core_releases/blob/master/latest'; then
        rm "$TMP_FILE"
    ...
    DOWNLOAD_LINK="https://github.com/cyberxsboy/v2ray-download/blob/master/v2ray-linux-64.zip"
